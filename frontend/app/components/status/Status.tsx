 ...
 * Status Component
 * 
 * Displays current agent status with comprehensive progress information, planned steps,
 * entities created, progress log, and failures. Updates automatically when status 
 * messages are received from the session.
 */
...
import { useMessages } from "@/utils/session";
...
/**
 * Status component for displaying comprehensive agent status
 * 
 * @param props - Component props including session ID
 * @returns JSX element representing the current status with detailed information
 */
export const Status: React.FC<StatusProps> = ({ sessionId }) => {
  const { messages } = useMessages(sessionId);
  const [currentStatus, setCurrentStatus] = useState<StateUpdateData | null>(null);

  // Find the latest status update from messages
  useEffect(() => {
    ...
    // Find the most recent state_update message
    ...
    if (statusMessages.length > 0) {
      ...
      setCurrentStatus(latestStatus.data as unknown as StateUpdateData);
    } else {
      setCurrentStatus(null);
    }
  }, [messages]);
